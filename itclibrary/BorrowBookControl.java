...
public class BorrowBookControl {
	...
	private ControlState controlState; //Change enum name  and variable name  (Orig: enum CONTROL_STATE)  (To:controlState; Orig:State;)
	...
	public BorrowBookControl() { // default constructor
		...
		controlState = ControlState.INITIALISED; //Changed enum name (Orig: enum CONTROL_STATE)  (To:controlState; Orig:State;)
	}


	public void setUI(BorrowBookUi ui) { // Change class name to BorrowBookUi (Orig: BorrowBookUI)
		if (!controlState.equals(ControlState.INITIALISED)) //Changed enum name (Orig: enum CONTROL_STATE)  (To:controlState; Orig:State;)
		...
		controlState = ControlState.READY; //Changed enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
	}


	public void swiped(int memberId) { // change parameter name and method name ( To: memberId; Orig: MEMMER_ID; , To: swiped; Orig: Swiped;)
		if (!controlState.equals(ControlState.READY)) //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
		...
		if (library.isMemberCanBorrow(member)) { // Change variable name( To: library;  Orig: LIBRARY; , To: member;  Orig: M;) Change method name (To:getMember; Orig:MEMBER_CAN_BORROW;)
			...
			controlState = ControlState.SCANNING; } //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
		else
		{
	...
	public void scanned(int bookId) { //Change method name (To:scanned; Orig:Scanned;)
		...
		if (!controlState.equals(ControlState.SCANNING)) { //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
	...
	public void complete() { //Change method name (To:complete; Orig:Complete;)
		if (pendingBookList.size() == 0) { // Change variable name ( To: pendingBookList; Orig: PENDING;)
			...
			controlState = ControlState.FINALISING; //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
		}
	}


	public void commitLoans() { //Change method name (To:commiLoans; Orig:Commit_LoOns;)
		if (!controlState.equals(ControlState.FINALISING)) { //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
		...
		controlState = ControlState.COMPLETED; //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
	}
	...
	public void cancel() {
		...
		controlState = ControlState.CANCELLED; //Change enum name (Orig: enum CONTROL_STATE) (To:controlState; Orig:State;)
	}
